...
// ActionUser "actioner". Both user ids are required. You pass the actionee's user id into the method and the actioner's is put into the
// ActionUser request object.
...
// ChangePassword Changes a user's password using the change password Id. This usually occurs after an email has been sent to the user
// ChangePassword and they clicked on a link to reset their password.
...
// ChangePasswordByIdentity Changes a user's password using their identity (login id and password). Using a loginId instead of the changePasswordId
// ChangePasswordByIdentity bypasses the email verification and allows a password to be changed directly without first calling the #forgotPassword
// ChangePasswordByIdentity method.
//   interface{} request The change password request that contains all of the information used to change the password.
...
// CreateApplicationRole Creates a new role for an application. You must specify the id of the application you are creating the role for.
// CreateApplicationRole You can optionally specify an Id for the role inside the ApplicationRole object itself, if not provided one will be generated.
...
// CreateAuditLog Creates an audit log with the message and user name (usually an email). Audit logs should be written anytime you
// CreateAuditLog make changes to the FusionAuth database. When using the FusionAuth App web interface, any changes are automatically
// CreateAuditLog written to the audit log. However, if you are accessing the API, you must write the audit logs yourself.
...
// CreateUserAction Creates a user action. This action cannot be taken on a user until this call successfully returns. Anytime after
// CreateUserAction that the user action can be applied to any user.
...
// CreateUserActionReason Creates a user reason. This user action reason cannot be used when actioning a user until this call completes
// CreateUserActionReason successfully. Anytime after that the user action reason can be used.
...
func (c *FusionAuthClient) DeactivateUsers(userIds []string) (interface{}, error) {
	...
	req, err := c.NewRequest(method, uri, body)
	q := req.URL.Query()
	q.Add("userId", string(userIds))
	var resp interface{}
	_, err = c.Do(req, &resp)
	return resp, err
}

// DeleteApplication Hard deletes an application. This is a dangerous operation and should not be used in most circumstances. This will
// DeleteApplication delete the application, any registrations for that application, metrics and reports for the application, all the
// DeleteApplication roles for the application, and any other data associated with the application. This operation could take a very
// DeleteApplication long time, depending on the amount of data in your database.
...
func (c *FusionAuthClient) DeleteApplication(applicationId string) (interface{}, error) {
	...
	req, err := c.NewRequest(method, uri, body)
	q := req.URL.Query()
	q.Add("hardDelete", string(true))
	var resp interface{}
	_, err = c.Do(req, &resp)
	return resp, err
}

// DeleteApplicationRole Hard deletes an application role. This is a dangerous operation and should not be used in most circumstances. This
// DeleteApplicationRole permanently removes the given role from all users that had it.
//   string applicationId The Id of the application to deactivate.
...
// DeleteUser Deletes the user for the given Id. This permanently deletes all information, metrics, reports and data associated
// DeleteUser with the user.
...
// DeleteUserAction Deletes the user action for the given Id. This permanently deletes the user action and also any history and logs of
// DeleteUserAction the action being applied to any users.
...
// GenerateEmailVerificationId Generate a new Email Verification Id to be used with the Verify Email API. This API will not attempt to send an
// GenerateEmailVerificationId email to the User. This API may be used to collect the verificationId for use with a third party system.
...
func (c *FusionAuthClient) GenerateEmailVerificationId(email string) (interface{}, error) {
	...
	q.Add("sendVerifyPasswordEmail", string(false))
	var resp interface{}
	_, err = c.Do(req, &resp)
	return resp, err
}

// GenerateRegistrationVerificationId Generate a new Application Registration Verification Id to be used with the Verify Registration API. This API will not attempt to send an
// GenerateRegistrationVerificationId email to the User. This API may be used to collect the verificationId for use with a third party system.
//   string email The email address of the user that needs a new verification email.
...
// GenerateTwoFactorSecret Generate a Two Factor secret that can be used to enable Two Factor authentication for a User. The response will contain
// GenerateTwoFactorSecret both the secret and a Base32 encoded form of the secret which can be shown to a User when using a 2 Step Authentication
// GenerateTwoFactorSecret application such as Google Authenticator.
...
// GenerateTwoFactorSecretUsingJWT Generate a Two Factor secret that can be used to enable Two Factor authentication for a User. The response will contain
// GenerateTwoFactorSecretUsingJWT both the secret and a Base32 encoded form of the secret which can be shown to a User when using a 2 Step Authentication
// GenerateTwoFactorSecretUsingJWT application such as Google Authenticator.
...
// IdentityProviderLogin Handles login via third-parties including Social login, external OAuth and OpenID Connect, and other
// IdentityProviderLogin login systems.
...
// ImportUsers Bulk imports multiple users. This does some validation, but then tries to run batch inserts of users. This reduces
// ImportUsers latency when inserting lots of users. Therefore, the error response might contain some information about failures,
// ImportUsers but it will likely be pretty generic.
...
// IssueJWT Issue a new access token (JWT) for the requested Application after ensuring the provided JWT is valid. A valid
// IssueJWT access token is properly signed and not expired.
// IssueJWT <p>
// IssueJWT This API may be used in an SSO configuration to issue new tokens for another application after the user has
// IssueJWT obtained a valid token from authentication.
...
// LoginPing Sends a ping to FusionAuth indicating that the user was automatically logged into an application. When using
// LoginPing FusionAuth's SSO or your own, you should call this if the user is already logged in centrally, but accesses an
// LoginPing application where they no longer have a session. This helps correctly track login counts, times and helps with
// LoginPing reporting.
...
// Logout The Logout API is intended to be used to remove the refresh token and access token cookies if they exist on the
// Logout client and revoke the refresh token stored. This API does nothing if the request does not contain an access
// Logout token or refresh token cookies.
...
// LookupIdentityProvider Retrieves the identity provider for the given domain. A 200 response code indicates the domain is managed
// LookupIdentityProvider by a registered identity provider. A 404 indicates the domain is not managed.
...
// ModifyAction Modifies a temporal user action by changing the expiration of the action and optionally adding a comment to the
// ModifyAction action.
...
// Register Registers a user for an application. If you provide the User and the UserRegistration object on this request, it
// Register will create the user as well as register them for the application. This is called a Full Registration. However, if
// Register you only provide the UserRegistration object, then the user must already exist and they will be registered for the
// Register application. The user id can also be provided and it will either be used to look up an existing user or it will be
// Register used for the newly created User.
...
// RetrieveActions Retrieves all of the actions for the user with the given Id. This will return all time based actions that are active,
// RetrieveActions and inactive as well as non-time based actions.
...
func (c *FusionAuthClient) RetrieveActionsPreventingLogin(userId string) (interface{}, error) {
	...
	q := req.URL.Query()
	q.Add("userId", string(userId))
	q.Add("preventingLogin", string(true))
	var resp interface{}
	_, err = c.Do(req, &resp)
	return resp, err
}

// RetrieveActiveActions Retrieves all of the actions for the user with the given Id that are currently active.
// RetrieveActiveActions An active action means one that is time based and has not been canceled, and has not ended.
//   string userId The Id of the user to fetch the actions for.
...
func (c *FusionAuthClient) RetrieveAuditLog(auditLogId int) (interface{}, error) {
	...
	uri := "/api/system/audit-log"
	method := http.MethodGet
	uri = URIWithSegment(uri, auditLogId)
	req, err := c.NewRequest(method, uri, body)
	q := req.URL.Query()
	var resp interface{}
	_, err = c.Do(req, &resp)
...
// RetrieveDailyActiveReport Retrieves the daily active user report between the two instants. If you specify an application id, it will only
// RetrieveDailyActiveReport return the daily active counts for that application.
//   string applicationId (Optional) The application id.
...
// RetrieveEmailTemplatePreview Creates a preview of the email template provided in the request. This allows you to preview an email template that
// RetrieveEmailTemplatePreview hasn't been saved to the database yet. The entire email template does not need to be provided on the request. This
// RetrieveEmailTemplatePreview will create the preview based on whatever is given.
...
// RetrieveInactiveActions Retrieves all of the actions for the user with the given Id that are currently inactive.
// RetrieveInactiveActions An inactive action means one that is time based and has been canceled or has expired, or is not time based.
//   string userId The Id of the user to fetch the actions for.
...
// RetrieveJWTPublicKey Retrieves the Public Key configured for verifying JSON Web Tokens (JWT) by the key Id. If the key Id is provided a
// RetrieveJWTPublicKey single public key will be returned if one is found by that id. If the optional parameter key Id is not provided all
// RetrieveJWTPublicKey public keys will be returned.
...
// RetrieveLoginReport Retrieves the login report between the two instants. If you specify an application id, it will only return the
// RetrieveLoginReport login counts for that application.
//   string applicationId (Optional) The application id.
//   interface{} start The start instant as UTC milliseconds since Epoch.
//   interface{} end The end instant as UTC milliseconds since Epoch.
...
// RetrieveMonthlyActiveReport Retrieves the monthly active user report between the two instants. If you specify an application id, it will only
// RetrieveMonthlyActiveReport return the monthly active counts for that application.
//   string applicationId (Optional) The application id.
//   interface{} start The start instant as UTC milliseconds since Epoch.
//   interface{} end The end instant as UTC milliseconds since Epoch.
...
// RetrieveRegistrationReport Retrieves the registration report between the two instants. If you specify an application id, it will only return
// RetrieveRegistrationReport the registration counts for that application.
//   string applicationId (Optional) The application id.
//   interface{} start The start instant as UTC milliseconds since Epoch.
//   interface{} end The end instant as UTC milliseconds since Epoch.
...
// RetrieveTotalReport Retrieves the totals report. This contains all of the total counts for each application and the global registration
// RetrieveTotalReport count.
...
// RetrieveUserAction Retrieves the user action for the given Id. If you pass in null for the id, this will return all of the user
// RetrieveUserAction actions.
...
// RetrieveUserActionReason Retrieves the user action reason for the given Id. If you pass in null for the id, this will return all of the user
// RetrieveUserActionReason action reasons.
...
// RetrieveUserByChangePasswordId Retrieves the user by a change password Id. The intended use of this API is to retrieve a user after the forgot
// RetrieveUserByChangePasswordId password workflow has been initiated and you may not know the user's email or username.
...
// RetrieveUserByVerificationId Retrieves the user by a verificationId. The intended use of this API is to retrieve a user after the forgot
// RetrieveUserByVerificationId password workflow has been initiated and you may not know the user's email or username.
...
// RetrieveUserLoginReport Retrieves the login report between the two instants for a particular user by Id. If you specify an application id, it will only return the
// RetrieveUserLoginReport login counts for that application.
//   string applicationId (Optional) The application id.
...
// RetrieveUserLoginReportByLoginId Retrieves the login report between the two instants for a particular user by login Id. If you specify an application id, it will only return the
// RetrieveUserLoginReportByLoginId login counts for that application.
//   string applicationId (Optional) The application id.
...
// RevokeRefreshToken Revokes a single refresh token, all tokens for a user or all tokens for an application. If you provide a user id
// RevokeRefreshToken and an application id, this will delete all the refresh tokens for that user for that application.
...
// SendEmail Send an email using an email template id. You can optionally provide <code>requestData</code> to access key value
// SendEmail pairs in the email template.
...
// ValidateJWT Validates the provided JWT (encoded JWT string) to ensure the token is valid. A valid access token is properly
// ValidateJWT signed and not expired.
// ValidateJWT <p>
// ValidateJWT This API may be used to verify the JWT as well as decode the encoded JWT into human readable identity claims.
//   string encodedJWT The encoded JWT (access token).
