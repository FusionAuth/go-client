...
func (c *FusionAuthClient) CreateApplication(applicationId string, request ApplicationRequest) (*ApplicationResponse, *Errors, error) {
	...
	restClient := c.Start(&resp, &errors)
	restClient = restClient.WithUri("/api/application").
		WithUriSegment(applicationId).
		WithJSONBody(request).
		WithMethod(http.MethodPost)

	if request.Application.TenantId != "" {
		restClient = restClient.WithHeader("X-FusionAuth-TenantId", request.Application.TenantId)
	}
